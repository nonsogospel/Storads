...
from .models import Post, Comments, Like, Contactus
...
def post_detail(request, pk):
	...
	if request.method == 'POST':
		...
		form = NewCommentForm()
	return render(request, 'account/post_detail.html', {'post':post, 'is_liked':is_liked, 'form':form})
...
class PostUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
	model = Post
	fields = ['name', 'pic', 'tags']
	template_name = 'account/create_post.html'
...
def search_posts(request):
	...
	return render(request, "account/search_posts.html", context)
...
def contact_process(request):
	if request.method == 'POST':
		...
		contactuser = Contactus(name=name, email=email, subject=subject, message=message)
		contactuser.save();

		messages.info(request, 'Thank You!. Your Message is successfully sent!')
		return render(request, 'contact.html')
	else:
		return render(request, 'contact.html')
